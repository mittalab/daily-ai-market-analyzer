...
export default function AnalyseScreen({ active }: { active: boolean }) {
  ...
  const filteredNifty50 = niftySymbols.filter(sym =>
    sym.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const filteredOthers = foStocks.filter(sym =>
    !niftySymbols.includes(sym) && sym.toLowerCase().includes(searchTerm.toLowerCase())
  ).sort();
        ...
        <div className="md:col-span-7">
          ...
          {result && analysis ? (
                      ...
                      <Expander title="Scoring Breakdown">
                        <div className="space-y-2.5 pt-1">
                          {SCORING_ROWS.map(row => {
                            const score = analysis.scoring_breakdown?.[row.key] ?? 0;
            ...
            <div className="hidden md:flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-xl p-8 text-center bg-gray-50/30 h-[300px]">
              ...
              <p className="text-sm font-semibold text-gray-500">Ready to Analyze</p>
              <p className="text-xs text-gray-400 mt-1">Select a stock symbol on the left to start real-time market analysis.</p>
            </div>
          )}
        </div>
      </div>
